...
/**
 * Defines application features from Select2 context.
 */
class Select2Context extends BaseContext
{
    ...
    private function fillSearchField(DocumentElement $page, $field, $value)
    {
        ...
        if ('Behat\Mink\Driver\Selenium2Driver' === get_class($driver)) {
            ...
            $select2Input = $this->getSession()->getDriver()->getWebDriverSession()->element('xpath', "//html/descendant-or-self::*[@class and contains(concat(' ', normalize-space(@class), ' '), ' select2-input ')]");
            if (!$select2Input) {
                throw new \Exception(sprintf('No field "%s" found', $field));
            }
            $select2Input->postValue(['value' => [$value]]);
        } else {
            $select2Input = $page->find('css', '.select2-input');
            if (!$select2Input) {
